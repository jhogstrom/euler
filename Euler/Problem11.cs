namespace Euler
{
    ...
    internal class Problem11 : EulerProblem
    {
        ...
        public Problem11(Printing printing) : base(printing)
        {
        }
        ...
        protected override long GetCalculationResult()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                for (int j = 0; j < 20; j++)
                {
                    var r = CalculateHighestProduct(i, j);
                    if (r > best)
                    {
                        best = r;
                    }
        ...
        private static long CalculateProductUsingStrategy(Func<int, int, Queue<long>> strategy, int x, int y)
        {
            return strategy(x, y).Product();
        }

        private long CalculateHighestProduct(int x, int y)
        {
            long best = CalculateProductUsingStrategy(HorizontalStrategy, x, y);
            var res = CalculateProductUsingStrategy(VerticalStrategy, x, y);
            if (res > best)
            {
                best = res;
            }

            res = CalculateProductUsingStrategy(DiagonalStrategyDownLeft, x, y);
            if (res > best)
            {
                best = res;
            }

            res = CalculateProductUsingStrategy(DiagonalStrategyDownRight, x, y);
            if (res > best)
            {
                best = res;
            }
        ...
        private Queue<long> DiagonalStrategyDownLeft(int x, int y)
        {
            var result = new Queue<long>();
            for (int i = 0; i < Cellcount; i++)
            {
                result.Enqueue(this.TryExtractValue(x - i, y + i));
            }

            return result;
        }

        private Queue<long> DiagonalStrategyDownRight(int x, int y)
        {
            var result = new Queue<long>();
            for (int i = 0; i < Cellcount; i++)
            {
        ...
        private Queue<long> VerticalStrategy(int x, int y)
        {
            var result = new Queue<long>();
            for (int i = 0; i < Cellcount; i++)
            {
                result.Enqueue(TryExtractValue(x, y + i));
            }

            return result;
        }

        private Queue<long> HorizontalStrategy(int x, int y)
        {
            var result = new Queue<long>();
            for (int i = 0; i < Cellcount; i++)
            {
                result.Enqueue(TryExtractValue(x + i, y));
            }

            return result;
        }

        private long TryExtractValue(int x, int y)
        {
            if (x >= 0 && y >= 0 && x < _matrix.Count && y < _matrix[x].Count)
            {
                return _matrix[x][y];
            }
